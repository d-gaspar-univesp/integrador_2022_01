...
var yield = {};
var yield_T1 = {"date": [], "yield": [], "color": []};
var yield_T2 = {"date": [], "yield": [], "color": []};
var yield_T3 = {"date": [], "yield": [], "color": []};
  ...
  resp.on('end', () => {
    ...
    for (date in yield) {
      if ((yield[date]["T1"]["B"] + yield[date]["T1"]["E"]) == 0) {
        ...
        if (yield_T1["yield"][yield_T1["yield"].length-1] < 0.85) {
          yield_T1["color"].push("rgba(255,0,0,1)");
        } else {
          yield_T1["color"].push("rgba(0,255,0,1)");
        }
      }

      if ((yield[date]["T2"]["B"] + yield[date]["T2"]["E"]) == 0) {
        yield_T2["date"].push(date);
        yield_T2["yield"].push(0);
        yield_T2["color"].push("rgba(85,85,85,1)");
      } else {
        yield_T2["date"].push(date);
        yield_T2["yield"].push(yield[date]["T2"]["B"] / (yield[date]["T2"]["B"] + yield[date]["T2"]["E"]));
        if (yield_T2["yield"][yield_T2["yield"].length-1] < 0.85) {
          yield_T2["color"].push("rgba(255,0,0,1)");
        } else {
          yield_T2["color"].push("rgba(0,255,0,1)");
        }
      }
            
      if ((yield[date]["T3"]["B"] + yield[date]["T3"]["E"]) == 0) {
        yield_T3["date"].push(date);
        yield_T3["yield"].push(0);
        yield_T3["color"].push("rgba(85,85,85,1)");
      } else {
        yield_T3["date"].push(date);
        yield_T3["yield"].push(yield[date]["T3"]["B"] / (yield[date]["T3"]["B"] + yield[date]["T3"]["E"]));
        if (yield_T3["yield"][yield_T3["yield"].length-1] < 0.85) {
          yield_T3["color"].push("rgba(255,0,0,1)");
        } else {
          yield_T3["color"].push("rgba(0,255,0,1)");
        }
      }
    }
